...
cmds=(
	...
	-n "evix distributed remote=4" "$evix eval --no-daemon --workers 0 --remote 127.0.0.1:$port $sys 4$args_str >/dev/null"
	-n "evix distributed local=4 remote=4" "$evix eval --no-daemon --workers 4 --remote 127.0.0.1:$port $sys 4$args_str >/dev/null"
